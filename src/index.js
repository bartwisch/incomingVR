...
function onFrame(delta, _time, { controllers, camera, player }) {
	...
	const enemyFactories = bulletGroup?.userData?.enemyFactories;
	if (enemyGroup && enemyFactories?.length) {
		...
		while (enemySpawnTimer >= ENEMY_SPAWN_INTERVAL) {
			...
			const factory =
				enemyFactories[Math.floor(Math.random() * enemyFactories.length)];
			const enemy = factory();
